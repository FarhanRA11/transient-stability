...
Berdasarkan persamaan ayunan persamaan \ref{Eq: Pswing}, nilai koefisien redaman ($\mathit{D}$) dan konstanta inersia ($\mathit{J}$) juga dapat diimplementasikan ke inverter dalam bentuk penguatan terhadap deviasi dan perubahan frekuensi. Maka dari itu, bisa didapatkan persamaan untuk menghitung daya akibat \textit{virtual inertia} seperti pada persamaan \ref{Eq: PVI}.
...
\begin{equation}
	\Delta P = J\frac{d\omega}{dt} + D(\omega - \omega_n)
\end{equation}
...
\end{equation}
\begin{equation}
	P_{VI} = -J_{virt}\frac{d\omega}{dt} -D_{virt}(\omega - \omega_n)
\label{Eq: PVI}
\end{equation}
...
\end{equation}
\begin{equation}
	P_{ref} = P_{ref,0} -J_{virt}\frac{d\omega}{dt} -D_{virt}(\omega - \omega_n)
\end{equation}
...
kondisi \ sistem :
\begin{cases}
      pra \ gangguan & \text{, } t < 1 \ s \\
      saat \ gangguan & \text{, } 1 \leq t \leq 1,02 \ s \\
	  pasca \ gangguan & \text{, } t > 1,02 \ s \\
\end{cases}
\]
...
\begin{enumerate}
	\item \textit{Overshoot} frekuensi, $\Delta f$ (Hz): nilai deviasi maksimum frekuensi di puncak atas dan bawah.
	\item RoCoF (\textit{Rate of Change of Frequency}), $\dot{f}$ (Hz/s): nilai maksimum dan minimum dari laju perubahan frekuensi. Diambil dari puncak atas dan bawah pada plot turunan frekuensi.
	\item Deviasi Sudut Rotor, $\Delta\delta$ (rad): nilai deviasi absolut maksimum sudut rotor dari nilai referensi, relatif terhadap generator 3.
	\item \textit{Settling Time} osilasi sudut rotor, $t_s$ (s): waktu yang dibutuhkan untuk osilasi sudut rotor stabil dengan kriteria kestabilan 2\%. 
	\item Frekuensi Teredam osilasi sudut rotor, $f_d$ (Hz): frekuensi osilasi yang telah teredam. Frekuensi ini dapat diestimasi dengan pendekatan FFT, lalu diambil frekuensi kritis, dalam konteks sistem tenaga listrik, diambil frekuensi terendah yang muncul pada spektrum frekuensi.
	\item Rasio Redaman osilasi sudut rotor, $\zeta$ (\%): seberapa cepat osilasi sudut rotor meredam. Rasio redaman diestimasi dari persamaan \ref{Eq: zeta}.
	
	\begin{equation}
		\zeta = \frac{4}{\sqrt{(\omega_d t_s)^2 + 16}}
	\label{Eq: zeta}
	\end{equation}
\end{enumerate}